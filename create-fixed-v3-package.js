...
// 输出文件路径
const outputPath = path.join(outputDir, 'zhiliuhuaxie-extension-fixed-v3.zip');
...
const fixPopupJs = (content) => {
  ...
  // 修复清空记录功能
  console.log('修复清空记录功能...');
  
  // 查找清空记录按钮的事件监听器
  const clearButtonRegex = /document\.getElementById\(['"]clearButton['"]\)\.addEventListener\(['"]click['"]/;
  if (clearButtonRegex.test(fixedContent)) {
    console.log('找到清空记录按钮事件监听器，检查实现...');
  } else {
    console.log('未找到清空记录按钮事件监听器，添加实现...');
    
    // 在文档加载完成后添加清空记录功能
    const docReadyRegex = /document\.addEventListener\(['"]DOMContentLoaded['"]/;
    if (docReadyRegex.test(fixedContent)) {
      fixedContent = fixedContent.replace(
        /document\.addEventListener\(['"]DOMContentLoaded['"], function\s*\(\s*\)\s*\{/,
        `document.addEventListener('DOMContentLoaded', function() {
  // 添加清空记录按钮事件监听器
  const clearButton = document.getElementById('clearButton');
  if (clearButton) {
    clearButton.addEventListener('click', function() {
      console.log('清空记录按钮被点击');
...
`
  ...
  // 确保有updateUI函数
  if (!/function\s+updateUI\s*\(\s*\)/.test(fixedContent)) {
    console.log('添加updateUI函数...');
    fixedContent += `
// 更新UI显示
function updateUI() {
  chrome.storage.local.get(['screenshots', 'clicks', 'recordingTime'], function(data) {
    const screenshots = data.screenshots || [];
    const clicks = data.clicks || [];
    const recordingTime = data.recordingTime || 0;
...
`;
  ...
  // 确保有showToast函数
  if (!/function\s+showToast\s*\(\s*message\s*\)/.test(fixedContent)) {
    console.log('添加showToast函数...');
    fixedContent += `
// 显示提示消息
...
const versionInfo = {
  ...
  buildId: buildId,
  buildTime: timestamp,
  features: ['auto-screenshot', 'click-tracking', 'document-generation', 'clear-records'],
  protection: {
    obfuscated: true,
    antiDebug: true,
    integrityCheck: true,
